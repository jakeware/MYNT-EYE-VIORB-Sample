...
bool StereoMsgSynchronizer::getRecentMsgs(sensor_msgs::ImageConstPtr &left_imgmsg, sensor_msgs::ImageConstPtr &right_imgmsg, std::vector<sensor_msgs::ImuConstPtr> &vimumsgs)
{
    ...
    if(_leftImageMsgQueue.empty() || _rightImageMsgQueue.empty())
    {
    ...
    {
        sensor_msgs::ImageConstPtr left_imsg;
        sensor_msgs::ImageConstPtr right_imsg;
        sensor_msgs::ImuConstPtr bmsg;

        //
        left_imsg = _leftImageMsgQueue.back();
        right_imsg = _rightImageMsgQueue.back();
        bmsg = _imuMsgQueue.front();

        // Check dis-continuity, tolerance 3 seconds
        if(left_imsg->header.stamp.toSec()-_imageMsgDelaySec + 3.0 < bmsg->header.stamp.toSec() )
        {
        ...
        //
        left_imsg = _leftImageMsgQueue.front();
        right_imsg = _rightImageMsgQueue.front();
        bmsg = _imuMsgQueue.back();

        // Wait imu messages in case communication block
        if(left_imsg->header.stamp.toSec()-_imageMsgDelaySec > bmsg->header.stamp.toSec())
        {
            return false;
        }

        // Check dis-continuity, tolerance 3 seconds
        if(left_imsg->header.stamp.toSec()-_imageMsgDelaySec > bmsg->header.stamp.toSec() + 3.0)
        {
            ROS_ERROR("Data dis-continuity, > 3 seconds. Buffer cleared");
            clearMsgs();
            return false;
        ...
        if(_leftImageMsgQueue.size()<10 && _rightImageMsgQueue.size()<10 && _imuMsgQueue.size()<15
           && left_imsg->header.stamp.toSec()-_imageMsgDelaySec>bmsg->header.stamp.toSec() )
        {
    ...
    // get image message
    left_imgmsg = _leftImageMsgQueue.front();
    right_imgmsg = _rightImageMsgQueue.front();
    ...
    while(true)
    {
        ...
        if(tmpimumsg->header.stamp.toSec() < left_imgmsg->header.stamp.toSec() - _imageMsgDelaySec)
        {
...
void StereoMsgSynchronizer::addImageMsg(const sensor_msgs::ImageConstPtr &imgmsg, std::queue<sensor_msgs::ImageConstPtr>* image_queue)
{
...
void StereoMsgSynchronizer::leftImageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    addImageMsg(msg, &_leftImageMsgQueue);
}

void StereoMsgSynchronizer::rightImageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    addImageMsg(msg, &_rightImageMsgQueue);
}
